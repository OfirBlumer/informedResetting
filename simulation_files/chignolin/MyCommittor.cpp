...
//+PLUMEDOC PRINTANALYSIS MYCOMMITTOR
/*
...
stops the simulation when it is in one of the two, with probability of 0.5. In the log
...
std::random_device rd;  // @customEdition
std::mt19937 gen(rd()); // @customEdition
std::uniform_real_distribution<> dis(0.0, 1.0); // @customEdition
class MyCommittor :
  public ActionPilot,
...
private:
  ...
  unsigned basin;
  bool doNotStop;
  std::vector<double> stopprobabilities;
public:
  static void registerKeywords( Keywords& keys );
  explicit MyCommittor(const ActionOptions&ao);
  void calculate() override;
  void apply() override {}
};

PLUMED_REGISTER_ACTION(MyCommittor,"MYCOMMITTOR")

void MyCommittor::registerKeywords( Keywords& keys ) {
  ...
  keys.add("numbered", "BASIN_SP","Stopping probability for basin #");
...
MyCommittor::MyCommittor(const ActionOptions&ao):
  Action(ao),
  ActionPilot(ao),
  ...
  doNotStop(false)
{
  ...
  for(unsigned b=1;b<=nbasins; ++b ) {
    std::vector<double> tmpp;
    parseNumberedVector("BASIN_SP", b, tmpp );
    if( tmpp.empty() ){
      if( 1==b ){
        stopprobabilities.assign (nbasins,1);
        break;
      } else error("Number of BAISN_SP does not match number of basins");
    };
    stopprobabilities.push_back(tmpp[0]);
  }
...
void MyCommittor::calculate() {
  ...
  if(inonebasin&&(!doNotStop)) {
    ...
    if (rank==0){
      if (stopprobabilities[basin-1]>dis(gen)) stop = true;
    }
    MPI_Bcast(&stop, 1, MPI_CXX_BOOL, 0, MPI_COMM_WORLD);
    if (stop){ // @customEdition
